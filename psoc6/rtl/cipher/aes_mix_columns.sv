...
module aes_mix_columns (
    ...
    assign fwd_r0 = xtime(b0) ^ (xtime(b1) ^ b1) ^ b2 ^ b3;
    assign fwd_r1 = b0 ^ xtime(b1) ^ (xtime(b2) ^ b2) ^ b3;
    assign fwd_r2 = b0 ^ b1 ^ xtime(b2) ^ (xtime(b3) ^ b3);
    assign fwd_r3 = (xtime(b0) ^ b0) ^ b1 ^ b2 ^ xtime(b3);
