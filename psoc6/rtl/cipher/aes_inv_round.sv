...
// For the last inverse round, InvMixColumns is skipped.
...
module aes_inv_round (
    ...
    // Row 0: no shift
    assign isr[0]  = s_in[0];   assign isr[4]  = s_in[4];
    assign isr[8]  = s_in[8];   assign isr[12] = s_in[12];

    // Row 1: right shift by 1
    assign isr[1]  = s_in[13];  assign isr[5]  = s_in[1];
    assign isr[9]  = s_in[5];   assign isr[13] = s_in[9];

    // Row 2: right shift by 2 (= left shift by 2)
    assign isr[2]  = s_in[10];  assign isr[6]  = s_in[14];
    assign isr[10] = s_in[2];   assign isr[14] = s_in[6];

    // Row 3: right shift by 3
    assign isr[3]  = s_in[7];   assign isr[7]  = s_in[11];
    assign isr[11] = s_in[15];  assign isr[15] = s_in[3];
