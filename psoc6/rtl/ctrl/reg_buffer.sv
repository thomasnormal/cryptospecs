...
//
// 8 × 128-bit named blocks (BLOCK0–BLOCK7) used as the shared data path for
// AES, DES, and SHA cipher operations.  Block IDs 8, 9, 12 are virtual
// (LOAD_FIFO0, LOAD_FIFO1, STORE_FIFO) and are handled through the ld0/ld1
// staging inputs and st_data output below.
//
// Fixed AES block assignments (crypto_isa_pkg.sv §11.6):
//   BLOCK0 — plaintext in
//   BLOCK1 — ciphertext out
//   BLOCK4 — key[127:0]     (all key sizes)
//   BLOCK5 — key[255:128]   (AES-192/256 upper bits)
//
...
(
    ...
    logic [127:0] blocks [0:7];

    // Safe read: clamp to valid range; FIFO IDs return 0
    function automatic logic [127:0] rd_blk(input logic [3:0] id);
        return (id <= 4'd7) ? blocks[id] : 128'h0;
    endfunction
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            // Engine write
            if (eng_wr_en && eng_wr_addr <= 4'd7)
                blocks[eng_wr_addr] <= eng_wr_data;
